...
exports("getClothesData", function()
    return Clothes
end)
...
Citizen.CreateThread(function()
    for clothing, v in pairs(Config.Stores) do
        exports['rpx-core']:createPrompt(v.location..'-clothing', v.coords, 0xF3830D8E, 'Open ' .. v.name, { -- [J]
			type = 'client',
			event = 'rpx-appearance:client:openMenu',
			args = {false, 'clothingMenu'}
        })
		exports['rpx-core']:createPrompt(v.location..'-outfits', v.coords, 0xC7B5340A, 'Open Outfits', { -- ["ENTER"]
            type = 'client',
            event = 'rpx-appearance:client:openMenu',
...
RegisterNetEvent('rpx-appearance:client:newPlayer', function()
...
RegisterNetEvent('rpx-appearance:client:openMenu', function(newPlayer, menuType)
...
RegisterNUICallback('save', function()
    local playerPed = PlayerPedId()
    TriggerServerEvent('rpx-appearance:server:saveSkin', SkinData, ClothesData)
    if NewPlayer then
        NewPlayer = false
        DoScreenFadeOut(150)
        Citizen.Wait(500)
...
RegisterNUICallback('saveOutfit', function(data)
    ...
    TriggerServerEvent('rpx-appearance:server:saveOutfit', data.outfitName, clothes)
    DisableCamera()
end)
...
RegisterNUICallback('useOutfit', function(data)
	...
	local playerPed = PlayerPedId()
    TriggerServerEvent('rpx-appearance:server:saveSkin', SkinData, data.skin)
end)
...
OpenMenu = function(newPlayer, menuType)
    ...
    elseif menuType == 'outfitMenu' then
        ClothingRoomTransition(Config.StaticClothingRoom, true)
        exports['rpx-core']:TriggerCallback('rpx-appearance:server:getOutfits', function(outfits)
            SendNUIMessage({
                type = 'outfitMenu',
                outfits = outfits
            })
