        ...
        for k, v in pairs(data.overlays) do
            if k == "eyebrows" then
                ToggleOverlayChange(
                    ...
                    1.0,
                    0,
                    tonumber(v.color_palette),
                    tonumber(v.main_color),
                    tonumber(v.second_color),
                    tonumber(v.third_color),
                    tonumber(0),
                    tonumber(v.visibility / 100),
                    ped
                )
            end
        end
			...
			if v.visibility ~= 0 then
				...
				if v.tx_color_type == 0 then
					...
					Citizen.InvokeNative(
						...
						textureId,
						overlay_id,
						v.palette_color_primary,
						v.palette_color_secondary,
						v.palette_color_tertiary
					)
				end
