...
export default function App() {
  ...
  const rotateCamera = (e) => {
    fetch('https://rpx-appearance/rotateCamera', {method: 'POST', body: JSON.stringify({
      direction: e
    })})
  }

  const setCamera = (e) => {
    fetch('https://rpx-appearance/setCamera', {method: 'POST', body: JSON.stringify({
      direction: e
    })})
  }
  ...
  const closeMenu = (save) => {
    ...
    if(save) {
      fetch('https://rpx-appearance/closeMenu', {method: 'POST', body: JSON.stringify({})})
    } else {
      fetch('https://rpx-appearance/closeMenu2', {method: 'POST', body: JSON.stringify({})})
    }
  ...
  const saveCharacter = () => {
    if(navSelect != 1) {
      if(newPlayer) {
        fetch('https://rpx-appearance/save', {method: 'POST', body: JSON.stringify({})})
        closeMenu(true)
      } else if(!modal) {
        setModal(true)
      } else if(modal && outfitName.length > 1) {
        setModal(false)
        fetch('https://rpx-appearance/saveOutfit', {method: 'POST', body: JSON.stringify({
